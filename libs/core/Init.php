...
class Core_Init
{
    ...
    */
    ...
    private static function initBasicEnv()
    {
        ...
        // ODP预定义路径
        define('ROOT_PATH', '/usr/local/nginx/html/Demo/');
        
        // CONF_PATH是文件系统路径，不能传给Core_Conf
    ...
    private static function getAppName()
    {
        ...
        if(PHP_SAPI != 'cli')
        {
            ...
            // 注意这里 跟你的配置有关系如：[SCRIPT_FILENAME] => /var/www/html/levin/yaf/webroot/openapi/index.php
            if(count($script) == 8 && $script[7] == 'index.php')
            {
                $app_name = $script[6];
            }
        }
    ...
    private static function initAppEnv($app_name)
    {
        ...
        // 设置当前App
        require_once LIB_PATH . '/core/AppEnv.php';
        Core_AppEnv::setCurrApp(MAIN_APP);
    ...
    {
        // 读取App的ap框架配置
        require_once LIB_PATH . '/core/Conf.php';
        
        $conf = Core_Conf::getConf('application/product');
        
        // 设置代码目录，其他使用默认或配置值
        $conf['application']['directory'] = Core_AppEnv::getEnv('code');
